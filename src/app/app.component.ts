...
export class AppComponent {
  unsafeEval = false;  //------- modify this to toggle unsafe-eval for script src in CSP -------
  schema = `
  asyncapi: '2.0.0'
  ...
  constructor(private element: ElementRef, private meta: Meta) {
    this.meta.addTag({
      'http-equiv': 'Content-Security-Policy',
      content:
        `default-src 'none'; script-src 'self'${this.unsafeEval ? " 'unsafe-eval'" : ""}; connect-src 'self'; img-src 'self'; style-src 'self' 'unsafe-inline'; font-src 'self'; media-src 'self'; base-uri 'self'; form-action 'self';`,
    });
  }
  ngAfterViewInit() {
    ...
    AsyncApiStandalone.render({ schema: this.schema, config }, container);
  }
